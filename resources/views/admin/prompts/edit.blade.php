...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6"><h1>Редактирование промта</h1></div>
                <div class="col-sm-6">
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="card">
                ...
                <div class="card-body">
                    <form id="promptForm" action="{{ route('admin.prompts.update', $prompt->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="prompt_category_id" id="prompt_category_id" class="form-control">
                                        ...
                                        @foreach($categories as $category)
                                            ...
                                            <option value="{{ $category->id }}" {{ (string) old('prompt_category_id', $prompt->prompt_category_id) === (string) $category->id ? 'selected' : '' }}>
                                                {{ $d->name ?? '#'.$category->id }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="number" name="sort_order" id="sort_order" class="form-control" value="{{ old('sort_order', $prompt->sort_order) }}" min="0">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group pt-4">
                                    <input type="hidden" name="status" value="0">
                                    <label><input type="checkbox" name="status" value="1" {{ old('status', $prompt->status) ? 'checked' : '' }}> Активен</label>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="tab-content border border-top-0 p-3">
                            @foreach($languages as $index => $language)
                                @php
                                    $c = $language->code;
                                    $desc = $prompt->descriptions->firstWhere('language_id', $language->id);
                                @endphp
                                <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="lang-prompt-{{ $language->id }}">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="slug_{{ $c }}" id="slug_{{ $c }}" class="form-control @error('slug_'.$c) is-invalid @enderror" value="{{ old('slug_'.$c, $desc->slug ?? '') }}" data-slug-locked="{{ ($desc && ($desc->slug ?? '') !== '') ? '1' : '0' }}" autocomplete="off">
                                        @error('slug_'.$c)<div class="invalid-feedback">{{ $message }}</div>@enderror
                                    </div>
                                    <div class="form-group">
                                        <label for="excerpt_{{ $c }}">Короткое описание</label>
                                        <textarea name="excerpt_{{ $c }}" id="excerpt_{{ $c }}" class="form-control" rows="3">{{ old('excerpt_'.$c, $desc->excerpt ?? '') }}</textarea>
                                    </div>
                                    <div class="form-group mb-0">
                                        <label for="prompt_text_{{ $c }}">Текст промта @if($language->is_default)<span class="text-danger">*</span>@endif</label>
                                        <textarea name="prompt_text_{{ $c }}" id="prompt_text_{{ $c }}" class="form-control @error('prompt_text_'.$c) is-invalid @enderror" rows="10">{{ old('prompt_text_'.$c, $desc->prompt_text ?? '') }}</textarea>
