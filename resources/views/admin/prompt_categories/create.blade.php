        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <button type="submit" form="promptCategoryForm" class="btn btn-primary float-right">Сохранить</button>
                </div>
                <div class="card-body">
                    <form id="promptCategoryForm" action="{{ route('admin.prompt-categories.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select name="ai_field" id="ai_field" class="form-control @error('ai_field') is-invalid @enderror">
                                ...
                                @foreach($aiFieldOptions as $fieldKey => $fieldLabel)
                                    <option value="{{ $fieldKey }}" {{ old('ai_field') === $fieldKey ? 'selected' : '' }}>
                                        {{ $fieldLabel }} ({{ $fieldKey }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="row_data" id="row_data" class="form-control @error('row_data') is-invalid @enderror mt-2" rows="12">{{ old('row_data') }}</textarea>
